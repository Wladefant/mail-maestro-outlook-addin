...
class BrowserTracing {
  ...
  _createRouteTransaction(context) {
    ...
    if (isPageloadTransaction) {
      const sentryTrace = isPageloadTransaction
        ...
        : "";
      const baggage = isPageloadTransaction
        ? getMetaContent("baggage")
        : undefined;
  ...
  _registerInpInteractionListener() {
    const handleEntries = ({ entries }) => {
      ...
      const user =
        currentScope !== undefined ? currentScope.getUser() : undefined;
      entries.forEach((entry) => {
        if (isPerformanceEventTiming(entry)) {
          ...
          if (existingInteraction) {
            existingInteraction.duration = Math.max(
            ...
            duration >
              this._interactionIdToRouteNameMapping[minInteractionId].duration
          ) {
...
function getSource(context) {
  const sourceFromAttributes =
    context.attributes && context.attributes[SEMANTIC_ATTRIBUTE_SENTRY_SOURCE];
  // eslint-disable-next-line deprecation/deprecation
  const sourceFromData =
    context.data && context.data[SEMANTIC_ATTRIBUTE_SENTRY_SOURCE];
  // eslint-disable-next-line deprecation/deprecation
