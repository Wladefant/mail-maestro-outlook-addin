...
class BaseClient {
  ...
  captureEvent(event, hint, scope) {
    ...
    this._process(
      this._captureEvent(event, hint, capturedSpanScope || scope).then(
        (result) => {
          eventId = result;
        }
      )
  ...
  _prepareEvent(event, hint, scope, isolationScope = getIsolationScope()) {
    ...
    return prepareEvent(options, event, hint, scope, this, isolationScope).then(
      (evt) => {
        if (evt === null) {
          return evt;
        }
        ...
        if (!trace && propagationContext) {
          ...
          const dynamicSamplingContext = dsc
            ? dsc
            : getDynamicSamplingContextFromClient(trace_id, this, scope);
