    ...
    if (this.spanRecorder) {
      DEBUG_BUILD &&
        ...
        logger.log(
          ...
          new Date(endTimestampInS * 1000).toISOString(),
          this.op
        );
      ...
      this.spanRecorder.spans = this.spanRecorder.spans.filter((span) => {
        ...
        const { start_timestamp: startTime, timestamp: endTime } =
          spanToJSON(span);
        const spanStartedBeforeTransactionFinish =
          startTime && startTime < endTimestampInS;

        // Add a delta with idle timeout so that we prevent false positives
        const timeoutWithMarginOfError =
          (this._finalTimeout + this._idleTimeout) / 1000;
        const spanEndedBeforeFinalTimeout =
          endTime &&
          startTime &&
          endTime - startTime < timeoutWithMarginOfError;
